...
(function($) {
	...
	function AdvancedSlider(instance, options) {
		...
		var VERSION = '4.7.1',
			...
			isOldIE = (browserName === 'msie' && parseInt(browserVersion, 10) < 9),
		...
		*/
			...
			if (self.settings.xmlSource) {
				...
				$.ajax({type: 'GET',
						...
						success: function(data) {
							...
							slides = $('<div class="slides"></div>').appendTo(slider);
							xmlSlider = $(xml).find('slider');
							
							// find all the <slide> nodes
							$(xml).find('slide').each(function(index) {
								
								var slide = $('<div class="slide"></div>').appendTo(slides);
								...
								if (xmlThumbnail.length)
									if (xmlThumbnail.attr('type') === 'content')
										var thumbnail = $('<div></div>').html(xmlThumbnail.text())
		...
		function parseHTML() {
			...
			slides.children('.slide').each(function(index, slideElement) {
				// will contain data such as path, thumbnail, caption, or link
				
				var slide = {},
					slideIndex = self.settings.shuffle ? parseInt($(slideElement).data('index'), 10) : index;
		...
		function create() {
			...
			if (browserName === 'msie')
				if (parseInt(browserVersion, 10) === 6)
					slider.addClass('ie6');
				else if (parseInt(browserVersion, 10) === 7)
					slider.addClass('ie7');
				else if (parseInt(browserVersion, 10) === 8)
					slider.addClass('ie8');
				else if (parseInt(browserVersion, 10) === 9)
					slider.addClass('ie9');
			...
			slides.find('video').each(function() {
				...
				if (video.hasClass('video-js')) {
					var videojs =  window[ 'videojs' ] || window[ 'VideoJS' ];
				...
				slides.find('a.video').each(function() {
					...
					video.click(function(event) {
						...
						if (href.indexOf('youtube') !== -1 || href.indexOf('youtu.be') !== -1) {
							...
							var src = 'http://player.vimeo.com/video/'+ id +'?api=1&autoplay=1';
						...
						if (self.settings.leaveVideoAction === 'removeVideo' || isIOS || (browserName === 'msie' && parseInt(browserVersion, 10) <= 7))
							currentVideo = iframe;
						else
							checkAndSetupVideo();
			...
			if (hash) {
				...
				var values = hash.split('-'),
					slideIndex = parseInt(values.pop(), 10),
					sliderId = hash.slice(0, - slideIndex.toString().length - 1);
			...
			if (slider.find("a[rel^='slider-lightbox']").length)
				lightboxInstance = slider.find("a[rel^='slider-lightbox']").prettyPhoto({
					social_tools: '',
					deeplinking: false,
			...
			// disable the context menu for right clicks
			if (self.settings.disableRightClick) {
				slider.on('contextmenu', 'img, .thumbnail-scroller .thumbnail', function(e) {
					return false;
				});
			}
		...
		*/
		function doSliderLayout() {
			...
			if (self.settings.shadow) {
				...
				if ((fullscreenState === 'on' && !self.settings.fullscreenShadow) || sliderMain.width() < 400 ) {
					...
					var	shadowMiddle = shadow.find('.shadow-middle'),
						...
						shadowExtra = parseInt(shadow.css('height'), 10) + parseInt(shadow.css('margin-top'), 10);
			...
			if (self.settings.slideButtons) {
				...
				var buttonsExtra = parseInt(slideButtons.css('height'), 10) + parseInt(slideButtons.css('margin-top'), 10);
			...
			heightToExtract += parseInt(slideWrapper.css('border-top-width'), 10) + parseInt(slideWrapper.css('border-bottom-width'), 10);
			widthToExtract += parseInt(slideWrapper.css('border-left-width'), 10) + parseInt(slideWrapper.css('border-right-width'), 10);
			...
			slides.find('.slide').each(function(index) {
				...
				if (image.length) {
					if (image.data('loaded')) {
						...
						image.sliderImagesLoaded(function (images, proper, broken) {
							if (proper.length) {
		...
		function scaleAndAlign(image, alignType) {
			...
			switch (self.settings.scaleType) {
				...
				case 'proportionalFit':
					if (image.data('index') === currentIndex && image.data('loaded')) {
		...
		function createSlideButtons() {
			...
			for (var i = 0; i < numButtons; i++) {
				...
				button.click(function() {
					triggerType = 'button';
					gotoSlide(parseInt($(this).attr('rel'), 10));
				});
			}
		...
		*/
		function showThumbnail(index) {
			...
			if (isOldIE) {
				...
				thumbnailWrapper.find('.caption .background').animate({'opacity': 1}, self.settings.thumbnailSlideDuration);
			} else {
				thumbnailWrapper.animate({'top': topPos, 'opacity': 1},
											self.settings.thumbnailSlideDuration,
											self.settings.thumbnailSlideEasing);
		...
		*/
		function gotoSlide(index) {
			...
			if (isTransition || isImageLoading) {
				if (self.settings.overrideTransition) {
					switch(currentEffectType) {
						...
						case 'fade':
							...
							if (previousIndex !== -1)
								slideDivs[previousIndex].stop(false, true).css('visibility', '');
							break;
			...
			if (currentVideo) {
				...
				if ((self.settings.leaveVideoAction === 'removeVideo' || isIOS || (browserName === 'msie' && parseInt(browserVersion, 10) <= 7)) && videoHiddenImage.length) {
					var videoHiddenImage= slideDivs[currentIndex].find('.video-hidden-image');
			...
			if (image.length && (!image.data('loaded') || self.settings.scaleType === 'proportionalFit')) {
				isImageLoading = true;
				...
				if (self.settings.effectType === 'swipe') {
					...
					image.sliderImagesLoaded(function(images, proper, broken) {
						...
						if (currentIndex === image.data('index'))
							isImageLoading = false;
					});
		...
		function preloadNearbyImages(index) {
			...
			*/
			...
			if(numSlides <= 1) {
				...
				var next = (index === numSlides - 1) ? 0 : index + 1;
				
				var slideData = slidesData[next];
				
				if (slideData.lazyImage && !slideData.lazyImageLoaded) {
					var image = slideDivs[next].find('img.image'),
						newImage = $('<img class="image"/>').attr('src', slideData.lazyImage),
						alignType = slideData.properties.alignType;
					...
					newImage.data('index', next);
					
					if (image.length) {
						newImage.prependTo(image.parent());
						image.remove();
					} else {
						newImage.prependTo(slideDivs[next]);
					}
		...
		*/
		function completeTransition() {
			var slideData = slidesData[currentIndex],
				...
				captionDelay = parseInt(slideData.properties.captionDelay, 10);
			...
			if (lightboxLink.length && self.settings.lightboxIcon) {
				...
				lightboxIcon.click(function() {
					if (self.settings.effectType === 'swipe')
						lightboxLink.unbind('click.prettyphoto').bind('click.prettyphoto', {instance: lightboxInstance} , $.prettyPhoto.initialize);
			...
			slideDivs[currentIndex].children('.layer').not('.static, .init').each(function() {
				...
				var targetVertical = parseInt(layer.css(horizontalPosition), 10),
					targetHorizontal = parseInt(layer.css(verticalPosition), 10);
			...
			if ($.fn.videoController) {
				...
				if (videoElement) {
					if (!slidesData[currentIndex].videoInstance) {
						...
						slidesData[currentIndex].videoInstance = currentVideo;
		...
		*/
		function startFadeTransition() {
			var properties = slidesData[currentIndex].properties,
				...
				previousSlideElement,
				fadeInDuration = parseInt(properties.fadeInDuration, 10),
				fadeInEasing = properties.fadeInEasing,
				fadeOutDuration = parseInt(properties.fadeOutDuration, 10),
		...
		*/
		function startSlideTransition() {
			var properties = slidesData[currentIndex].properties,
				...
				slideDuration = parseInt(properties.slideDuration, 10),
			...
			if (previousIndex === -1) {
				...
				slides.css('overflow', 'visible')
						...
						.find('.slide').each(function(index) {
							...
							if (self.settings.swipeOrientation === 'horizontal')
								slide.css({'top': 0, 'left': index * (slides.width() + self.settings.swipeSlideDistance), 'visibility': 'visible'});
							else if (self.settings.swipeOrientation === 'vertical')
								slide.css({'top': index * (slides.height() + self.settings.swipeSlideDistance), 'left': 0, 'visibility': 'visible'});
						});
				...
				if (self.settings.swipeOrientation === 'horizontal') {
					...
					if (allowDrag)
						slides.swipe({
							...
							excludedElements: ".selectable, .noSwipe",
							...
							swipeStatus: function(event, phase, direction, distance) {
								...
								if (phase === 'start') {
									slidesPosition = parseInt(slides.css('left'), 10);
									...
									if (target.length && !('ontouchstart' in window)) {
							...
							click: function(event) {
								...
								if (target.length && !('ontouchstart' in window)) {
									...
									if (target.attr('rel') && $.prettyPhoto)
										target.unbind('click.prettyphoto').bind('click.prettyphoto', {instance: lightboxInstance} , $.prettyPhoto.initialize);
					...
					if (allowDrag)
						slides.swipe({
							...
							swipeStatus: function(event, phase, direction, distance) {
								if (phase === 'start') {
									slidesPosition = parseInt(slides.css('top'), 10);
		...
		*/
		function startSliceTransition() {
			var slideData = slidesData[currentIndex],
				...
				horizontalSlices = parseInt(properties.horizontalSlices, 10),
				verticalSlices = parseInt(properties.verticalSlices, 10),
				slicePattern = properties.slicePattern,
				slicePoint = properties.slicePoint,
				sliceStartPosition = properties.sliceStartPosition,
				sliceStartRatio = parseFloat(properties.sliceStartRatio, 10),
				sliceDuration = parseInt(properties.sliceDuration, 10),
				sliceEasing = properties.sliceEasing,
				sliceDelay = parseInt(properties.sliceDelay, 10),
				sliceFade = (properties.sliceFade === true || properties.effectType === 'fade') ? 0 : 1,
				
				fadePreviousSlide = properties.fadePreviousSlide,
				fadePreviousSlideDuration = parseInt(properties.fadePreviousSlideDuration, 10),
		...
		*/
		function animateSlice(slice, i, sliceEffectType, slicePoint, sliceStartPosition, sliceStartRatio, sliceDuration, sliceEasing, sliceDelay) {
			...
			var startState = {},
				...
				endWidth = parseInt(slice.css('width'), 10),
				endHeight = parseInt(slice.css('height'), 10),
				endLeft = parseInt(slice.css('left'), 10),
		...
		*/
		function showCaption() {
			...
			var slideData = slidesData[activeCaptionIndex],
				...
				captionShowEffectDuration = parseInt(properties.captionShowEffectDuration, 10),
			...
			if (captionShowEffect === "fade") {
				...
				if (captionShowSlideDirection === 'topToBottom')
					captionWrapper.css({'width': containerWidth, 'height': containerHeight, 'left': 0, 'top': -captionContainer.height()});
				else if (captionShowSlideDirection === 'bottomToTop')
					captionWrapper.css({'width': containerWidth, 'height': containerHeight, 'left': 0, 'top': captionContainer.height()});
				else if (captionShowSlideDirection === 'leftToRight')
					captionWrapper.css({'width': containerWidth, 'height': containerHeight, 'left': -captionContainer.width(), 'top': 0});
		...
		*/
		function hideCaption() {
			...
			var captionContainer = slideDivs[activeCaptionIndex].find('.caption-container'),
				...
				captionHideEffectDuration = parseInt(properties.captionHideEffectDuration, 10),
		...
		*/
		function stopTimerAnimation() {
			...
			if (timerAnimation) {
				if (!(isOldIE))
					timerAnimation.stop().animate({'opacity': 0}, self.settings.timerFadeDuration, function(){timerAnimation.remove();});
				else
		...
		*/
		function getLeftOffset(image, alignType) {
			...
			if (alignType === 'centerTop' || alignType === 'centerCenter' || alignType === 'centerBottom')
				left = slideWrapper.width() / 2 + parseInt(image.css('margin-left'), 10);
			else if (alignType === 'rightTop' || alignType === 'rightCenter' || alignType === 'rightBottom')
				left = slideWrapper.width() - parseInt(image.css('width'), 10);
		...
		*/
		function getTopOffset(image, alignType) {
			...
			if (alignType === 'leftCenter' || alignType === 'centerCenter' || alignType === 'rightCenter')
				top = slideWrapper.height() / 2 + parseInt(image.css('margin-top'), 10);
			else if (alignType === 'leftBottom' || alignType === 'centerBottom' || alignType === 'rightBottom')
				top = slideWrapper.height() - parseInt(image.css('height'), 10);
		...
		*/
		function createThumbnailScroller() {
			...
			thumbnailsVisibleContainer = $('<div class="thumbnails-visible-container"></div>').appendTo(thumbnailScroller);
			thumbnailsContainer = $('<div class="thumbnails-container"></div>').appendTo(thumbnailsVisibleContainer);
			...
			for (var i = 0; i < numSlides; i++) {
				if (slidesData[i].thumbnail && (slidesData[i].properties.thumbnailType === 'scroller' || slidesData[i].properties.thumbnailType === 'tooltipAndScroller')) {
					...
					var	thumbnailWrapper = $('<div class="thumbnail-wrapper"></li>').appendTo(thumbnailsContainer),
						thumbnail;
					...
					if (!self.settings.thumbnailGlow)
						thumbnail.css('boxShadow', 'none');

					if (!self.settings.thumbnailBackground)
						thumbnail.css('backgroundColor', 'transparent');
					...
					thumbnailsVisibleContainer.swipe({
						...
						swipeStatus: function(event, phase, direction, distance) {
							if (phase === 'start') {
								...
								scrollerPosition = parseInt(thumbnailsContainer.css('left'), 10);
					...
					thumbnailsVisibleContainer.swipe({
						...
						swipeStatus: function(event, phase, direction, distance) {
							...
							if (phase === 'start') {
								...
								scrollerPosition = parseInt(thumbnailsContainer.css('top'), 10);
		...
		*/
		function doThumbnailScrollerLayout() {
			...
			thumbnailScroller.find('.thumbnail-wrapper').each(function(index) {
				...
				thumbnailWrapper.css({'width': thumbnailWrapper.find('.thumbnail').outerWidth(false), 'height': thumbnailWrapper.find('.thumbnail').outerHeight(false)});
			...
			if (self.settings.thumbnailOrientation === 'horizontal') {
				var arrowsSize = self.settings.thumbnailArrows ? (parseInt(thumbnailScroller.css('paddingLeft'), 10) + parseInt(thumbnailScroller.css('paddingRight'), 10)) : 0;
				...
				var arrowsSize = self.settings.thumbnailArrows ? (parseInt(thumbnailScroller.css('paddingTop'), 10) + parseInt(thumbnailScroller.css('paddingBottom'), 10)) : 0;
		...
		*/
		function scrollToThumbnailPage(index, speed) {
			...
			thumbnailsContainer.animate(animObj, {duration: speed || self.settings.thumbnailScrollDuration,
		...
		*/
		function startThumbnailMouseScroll() {
			...
			var increment = 0,
				...
				thumbnailsPosition = parseInt(thumbnailsContainer.css(prop), 10);
		...
		function startThumbnailMouseWheel() {
			...
			thumbnailsVisibleContainer.bind('mousewheel', function(event, delta) {
				...
				if (!isThumbnailMouseWheelScrolling) {
					...
					currentPosition = parseInt(thumbnailsContainer.css(prop), 10);
		...
		*/
		function createScrollbar() {
			...
			if (self.settings.thumbnailOrientation === 'horizontal') {
				...
				thumbnailScrollerScrollbar.css({'top': thumbnailHeight * thumbnailLayers, 'left': (thumbnailScroller.outerWidth(false) - thumbnailScrollerScrollbar.width()) / 2});
				...
				thumbnailScrollerScrollbar.css({'left': thumbnailWidth * thumbnailLayers, 'top': (thumbnailScroller.outerHeight(false) - thumbnailScrollerScrollbar.height()) / 2});
			...
			bck.bind('click', function() {
				thumbPosition = parseInt(thumb.css(positionProp), 10) - self.settings.scrollbarArrowScrollAmount;
				move();
			});
			...
			function move() {
				...
				if (thumbPosition < 0)
					thumbPosition = 0;
				else if (thumbPosition > parseInt(thumbnailScrollerScrollbarTrack.css(sizeProp), 10) - parseInt(thumb.css(sizeProp), 10))
					thumbPosition =  parseInt(thumbnailScrollerScrollbarTrack.css(sizeProp), 10) - parseInt(thumb.css(sizeProp), 10);
				...
				scrollbarPosition = thumbPosition / (parseInt(thumbnailScrollerScrollbarTrack.css(sizeProp), 10) - parseInt(thumb.css(sizeProp), 10));
				...
				if (!isThumbnailScrollbarMoving) {
					...
					currentPosition = parseInt(thumbnailsContainer.css(positionProp), 10);
					
					thumbnailScrollbarTimer = setInterval(function() {
						if (Math.abs(getThumbnailsPosition() + scrollbarPosition) > 0.001) {
							var newPos = - scrollbarPosition * (thumbnailsTotalSize - parseInt(thumbnailsVisibleContainer.css(sizeProp), 10)),
		...
		*/
		function getThumbnailsPosition() {
			var positionProp = self.settings.thumbnailOrientation === 'horizontal' ? 'left' : 'top',
				sizeProp = self.settings.thumbnailOrientation === 'horizontal' ? 'width' : 'height',
				position = parseFloat(thumbnailsContainer.css(positionProp), 10) / (thumbnailsTotalSize - parseInt(thumbnailsVisibleContainer.css(sizeProp), 10));
		...
		function onThumbnailScrollProgress() {
			if (self.settings.thumbnailScrollbar && !isThumbnailScrollbarDragging) {
				var thumb = thumbnailScrollerScrollbarTrack.find('.thumb'),
					positionProp = self.settings.thumbnailOrientation === 'horizontal' ? 'left' : 'top',
					sizeProp = self.settings.thumbnailOrientation === 'horizontal' ? 'width' : 'height',
					maxValue = parseInt(thumbnailScrollerScrollbarTrack.css(sizeProp), 10) - parseInt(thumb.css(sizeProp), 10),
		...
		*/
		function showTooltip(index) {
			...
			var tooltipLeft = tooltip.outerWidth(false) / 2,
				tooltipTop = tooltip.outerHeight(false) + parseInt(tooltip.css('marginBottom'), 10);
		...
		*/
		...
		if (isIOS) {
			...
			if (iOSVersion.toString().indexOf('6.0') !== -1) {
...
/*changed name from imagesLoaded to sliderImagesLoaded to avoid conflicts*/
...
$.fn.sliderImagesLoaded = function( callback ) {
	var $this = this,
	...
	function imgLoaded( img, isBroken ) {
		...
		$.data( img, 'sliderImagesLoaded', { isBroken: isBroken, src: img.src } );
		...
		if ( $images.length === loaded.length ) {
			...
			$images.unbind( '.sliderImagesLoaded', imgLoadedHandler );
		}
	}
	...
	if ( !$images.length ) {
		...
		$images.bind( 'load.sliderImagesLoaded error.sliderImagesLoaded', imgLoadedHandler )
		.each( function( i, el ) {
			var src = el.src;

			// find out if this image has been already checked for status
			// if it was, and src has not changed, call imgLoaded on it
			var cached = $.data( el, 'sliderImagesLoaded' );
			...
			if ( el.readyState || el.complete ) {
				...
				if ( navigator.appName === 'Microsoft Internet Explorer' || navigator.userAgent.match(/Windows Phone/i) ) {
					var path = src.split('?'),
						number = typeof path[1] !== 'undefined' ? parseInt( path[1] ) + 1 : 1;

					el.src = path[0] + '?' + number;
				} else {
					el.src = src;
				}
			}
